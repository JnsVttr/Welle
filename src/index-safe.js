...
document.getElementById("checkMuteAlerts").checked = false;
...
function setUser(name, session) {
   	if (user == 'local') { 
		printer(debug, context, "setUser", `emit new User: "${name}" to server. Session: ${session}`)
   		socket.emit('setUser', name, session); // wait form confirmation
   		socket.emit("addUserToSocket", name, session);
   	} else { 
		printer(debug, context, "setUser", `no emitting, user already joined a session..`)
