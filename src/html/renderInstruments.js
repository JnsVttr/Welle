...
export function renderInstruments(_instruments) {
	...
	Object.keys(_instruments).forEach(inst => {
		...
		html+= `
		vol: 
		<input id="vol_${name}" type="text" style="max-width: 3rem" autofocus size="10" autocomplete=off value="${volume}">
		pattern: 
		<input id="pattern_${name}" type="text" autofocus style="max-width: 55rem" size="${length}" autocomplete=off value="${pattern}">
